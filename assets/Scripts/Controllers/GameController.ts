...
export default class GameController {
    ...
    public addStaticTetriminoBlocks(tetrimino: Tetrimino): void {
        ...
        if (tetrimino.node && tetrimino.node.isValid) {
            tetrimino.node.destroy();
        }
    ...
    public nudgeLeftOnce(): void {
        if (this.mCurrentTetrimino && this.mGameState === GameState.Running) {
            this.mCurrentTetrimino.nudgeLeftOnce();
        }
    }

    public nudgeRightOnce(): void {
        if (this.mCurrentTetrimino && this.mGameState === GameState.Running) {
            this.mCurrentTetrimino.nudgeRightOnce();
        }
    }

    public rotateOnce(): void {
        if (this.mCurrentTetrimino && this.mGameState === GameState.Running) {
            this.mCurrentTetrimino.rotateOnceByUser();
        }
    }

    public setSoftDropEnabled(enabled: boolean): void {
        if (this.mCurrentTetrimino && this.mGameState === GameState.Running) {
            this.mCurrentTetrimino.setSoftDropEnabled(enabled);
        }
    }
}
